...
def get_records_for_station_and_date(station_id: str, date: datetime.date) -> List[Tuple]:
    ...
    with CursorFromConnectionFromPool() as cursor:
        cursor.execute(
            "SELECT id, station_id, source_timestamp, temperature, wind_speed, max_wind_speed, wind_direction, rain, humidity, pressure, flagged, taken_timestamp, gatherer_run_id, cumulative_rain "
            "FROM weather_record "

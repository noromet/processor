...
def get_stations(station_id: str | None = None, all_stations: bool = False):
    ...
    if station_id is not None:
        stations = [Database.get_single_station(station_id)]
...
def main():
    ...
    with database_connection(args.db_url):
        ...
        save_funcs = {
            DailyRecord: Database.save_daily_record,
            MonthlyRecord: Database.save_monthly_record,
        }
        ...
        while not processing_queue.empty():
            ...
            if args.dry_run:
                ...
                save_func = save_funcs.get(type(result))
                if save_func:
                    save_func(result)
                    record_id = getattr(result, "dr_id", getattr(result, "mr_id", None))
                    logging.info("Inserted record with id: %s", record_id)
                else:
                    logging.error("Unknown result type: %s", type(result))

        logging.info("Cook done.")
